...
function autocomplete_insurance(inp, arr, arr_2) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          ...
          if (company_name[i].substr(0, val.length).toUpperCase() == val.toUpperCase()  || address1[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address2[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || city[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || state[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || zip[i].substr(0, val.length).toUpperCase() == val.toUpperCase() )  {
            ...
            //   b.innerHTML += ', '
            // }
            b.innerHTML +=  'Allstate Insurance: ';
            if (address1[i] != "") {
            ...
            b.addEventListener("click", function(e) {
                ...
                $(inp).parents('form').find('input[name="company_name"]').val(input_array[0].trim())
                try {
                  // Company Contact Panel
                $(inp).parents('form').find('input[name="name"]').val(input_array[0].trim())                
                ...
                $(inp).parents('form').find('input[name="phone"]').val(input_array[9].trim())
                } catch (error) {
                  
                }
                $(inp).parents('form').find('input[name="fax"]').val(input_array[10].trim())
                $(inp).parents('form').find('input[name="policy_number"]').val(input_array[11].trim())
                $(inp).parents('form').find('input[name="claim_number"]').val(input_array[12].trim())
                // console.log('input_array[11]',input_array)
                $(inp).parents('form').find('select[name="insurance_type_id"]').find('option').each(function() {
                    
                
                    if (parseInt($(this).val()) == parseInt(input_array[13])) {
                      // Set the selected property of the matched option to true
                      $(this).prop('selected', true);
                      // Break out of the loop since we found the matching option
                      return false;
                ...
                $(inp).parents('form').find('input[name="extension"]').val(input_array[14].trim())
                inp.value = "";
            });
            a.appendChild(b);
          }
...
function search_filter_insurance(this_element){
    console.log('Search filter function')
    var $this = $(this_element);
    $.ajax({ 
          method: "GET",
          url: `/30/search_filter_insurance/`,
          ...
          success:function(response){
            ...
            autocomplete_insurance(this_element, response.data, response.data );
