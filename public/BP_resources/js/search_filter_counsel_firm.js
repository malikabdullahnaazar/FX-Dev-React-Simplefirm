function format_phone_fax(phoneNumberInput) {
  ...
  function autocomplete_counsel_firm(inp, arr, arr_2) {
      ...
      inp.addEventListener("input", function(e) {
          ...
          for (i = 0; i < arr.length; i++) {
            ...
            if (firm_name[i].substr(0, val.length).toUpperCase() == val.toUpperCase()  || address1[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address2[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || city[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || state[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || zip[i].substr(0, val.length).toUpperCase() == val.toUpperCase() )  {
              ...
              b.innerHTML += "<input type='hidden' value='" + firm_name[i] +" % " + address1[i]+" % " + address2[i]+" % " + city[i]+" % "  + state[i] +" % "+  zip[i]+" % "+  email[i]+" % "+  phone[i]+" % "+ fax[i]+" % "+ extension[i]+ " % "+ website[i]+ " % "+ firm_id[i]+"'>";
              ...
              b.addEventListener("click", function(e) {
                  ...
                  try {                    
                  $(inp).parents('form').find('input[name="firm_name"]').val(input_array[0].trim())
                  } catch (error) {
                    
                  }
                  try {              
                  $(inp).parents('form').find('input[name="firm_address1"]').val(input_array[1].trim())
                  $(inp).parents('form').find('input[name="firm_address2"]').val(input_array[2].trim())
                  } catch (error) {
                    
                  }
                  $(inp).parents('form').find('input[name="firm_city"]').val(input_array[3].trim())
                  // $(inp).parents('form').find('input[name="state"]').val(input_array[6].trim())
                  try {
                    $(inp).parents('form').find('select[name="firm_state"]').find('option').each(function() {
                  ...
                  $(inp).parents('form').find('input[name="firm_zip"]').val(input_array[5].trim())
                  $(inp).parents('form').find('input[name="firm_email"]').val(input_array[6].trim())
                  $(inp).parents('form').find('input[name="firm_phone"]').val(input_array[7].trim())
                  
                  $(inp).parents('form').find('input[name="firm_fax"]').val(input_array[8].trim())
                  $(inp).parents('form').find('input[name="firm_extension"]').val(input_array[9].trim())
                  console.log('WEbiste',input_array[11].trim())
                  try {
                    
                    $(inp).parents('form').find('input[name="firm_website"]').val(input_array[10].trim())
                    } catch (error) {
                      
                    }
                    
                  $(inp).parents('form').find('input[name="firm_id"]').val(input_array[11].trim())
                  inp.value = "";                  
  ...
  function search_filter_counsel_firm(this_element){
      ...
      $.ajax({ 
            ...
            success:function(response){
              ...
              autocomplete_counsel_firm(this_element, response.data, response.data );
