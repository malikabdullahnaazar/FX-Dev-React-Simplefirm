function autocomplete_judge(inp, arr, arr_2) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          var judgename = arr_2.map( function(item){ return item.judgename })
          ...
          if (court_name[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || judgename[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address1[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address2[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || city[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || state[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || zip[i].substr(0, val.length).toUpperCase() == val.toUpperCase() )  {
            ...
            b.innerHTML += "<input type='hidden' value='" + judgename[i] +" , " + address1[i]+" , " + address2[i]+" , " + city[i]+" , "  + state[i] +" , "+  zip[i]+" , "+  email[i]+" , "+  phone[i]+" , "+ fax[i]+" , "+ extension[i]+" , "+ website[i]+ " , "+ court_id[i]+ "'>";
            ...
            b.addEventListener("click", function(e) {
                ...
                $(inp).parents('form').find('input[name="judge_name"]').val(input_array[0])
...
function search_filter_judge(this_element){
    // console.log('Search filter function')
    var $this = $(this_element);
    $.ajax({ 
          method: "GET",
          url: `/30/search_filter_judge/`,
        
          success:function(response){            
            autocomplete_judge(this_element, response.data, response.data );
