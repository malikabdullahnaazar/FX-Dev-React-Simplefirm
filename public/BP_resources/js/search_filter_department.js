function autocomplete_department(inp, arr, arr_2) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          var department = arr_2.map( function(item){ return item.department })
          ...
          if (court_name[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || department[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address1[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address2[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || city[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || state[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || zip[i].substr(0, val.length).toUpperCase() == val.toUpperCase() )  {
            ...
            b.innerHTML += "<input type='hidden' value='" + department[i] +" , " + address1[i]+" , " + address2[i]+" , " + city[i]+" , "  + state[i] +" , "+  zip[i]+" , "+  email[i]+" , "+  phone[i]+" , "+ fax[i]+" , "+ extension[i]+" , "+ website[i]+ " , "+ court_id[i]+ "'>";
            ...
            b.addEventListener("click", function(e) {
                ...
                $(inp).parents('form').find('input[name="department"]').val(input_array[0])
...
function search_filter_department(this_element){
    // console.log('Search filter function')
    var $this = $(this_element);
    $.ajax({ 
          method: "GET",
          url: `/30/search_filter_department/`,
        
          success:function(response){            
            autocomplete_department(this_element, response.data, response.data );
