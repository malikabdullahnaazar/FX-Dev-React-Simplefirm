function autocomplete_caseloan(inp, arr, arr_2) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          var company = arr_2.map( function(item){ return item.company })
          ...
          if (company[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address1[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address2[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || city[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || state[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || zip[i].substr(0, val.length).toUpperCase() == val.toUpperCase() )  {
            ...
            b.innerHTML += "<input type='hidden' value='" + company[i] +" , " + address1[i]+" , " + address2[i]+" , " + city[i]+" , "  + state[i] +" , "+  zip[i]+" , "+  email[i]+" , "+  phone[i]+" , "+ fax[i]+" , "+ extension[i]+" , "+ website[i]+ " , "+  report_type[i]+ "'>";
            ...
            b.addEventListener("click", function(e) {
                ...
                $(inp).parents('form').find('input[name="company_name"]').val(input_array[0])
...
function search_filter_caseloan(this_element){
    // console.log('Search filter function')
    var $this = $(this_element);
    $.ajax({ 
          method: "GET",
          url: `/30/search_filter_caseloan/`,
        
          success:function(response){            
            autocomplete_caseloan(this_element, response.data, response.data );
