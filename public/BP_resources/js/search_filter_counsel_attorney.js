function format_phone_fax(phoneNumberInput) {
  ...
  function autocomplete_counsel_attorney(inp, arr, arr_2) {
      ...
      inp.addEventListener("input", function(e) {
          ...
          for (i = 0; i < arr.length; i++) {
            var first_name = arr_2.map( function(item){ return item.first_name })            
            var last_name = arr_2.map( function(item){ return item.last_name })
            ...
            var attorney_id = arr_2.map( function(item){ return item.attorney_id })

            if (arr_2.map( function(item) { return item.first_name })[i] == null){
                first_name[i] = ''
              }
              if (arr_2.map( function(item) { return item.last_name })[i] == null){
                last_name[i] = ''
              }
            ...
            if (first_name[i].substr(0, val.length).toUpperCase() == val.toUpperCase()  || last_name[i].substr(0, val.length).toUpperCase() == val.toUpperCase()  || address1[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || address2[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || city[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || state[i].substr(0, val.length).toUpperCase() == val.toUpperCase() || zip[i].substr(0, val.length).toUpperCase() == val.toUpperCase() )  {
              ...
              if (arr_2.map( function(item) { return item.website })[i] == null){
                website[i] = ''
              }
              if (arr_2.map( function(item) { return item.attorney_id })[i] == null){
                attorney_id[i] = ''
              }
              ...
              if (first_name[i] != "") {
                b.innerHTML +=  first_name[i];
                b.innerHTML += ', '
              }
              if (last_name[i] != "") {
                b.innerHTML +=  last_name[i];
                b.innerHTML += ', '
              }
              if (address1[i] != "") {
              ...
              b.innerHTML += "<input type='hidden' value='" + first_name[i] +" % " + address1[i]+" % " + address2[i]+" % " + city[i]+" % "  + state[i] +" % "+  zip[i]+" % "+  email[i]+" % "+  phone[i]+" % "+ fax[i]+" % "+ extension[i]+ " % "+ website[i]+" % "+ last_name[i]+" % "+ attorney_id[i]+"'>";
              ...
              b.addEventListener("click", function(e) {
                  ...
                  try {                    
                  $(inp).parents('form').find('input[name="counsel_first_name"]').val(input_array[0].trim())
                  } catch (error) {
                    
                  }
                  try {              
                  $(inp).parents('form').find('input[name="counsel_address1"]').val(input_array[1].trim())
                  $(inp).parents('form').find('input[name="counsel_address2"]').val(input_array[2].trim())
                  } catch (error) {
                    
                  }
                  $(inp).parents('form').find('input[name="counsel_city"]').val(input_array[3].trim())
                  // $(inp).parents('form').find('input[name="state"]').val(input_array[6].trim())
                  try {
                    $(inp).parents('form').find('select[name="counsel_state"]').find('option').each(function() {
                  ...
                  $(inp).parents('form').find('input[name="counsel_zip"]').val(input_array[5].trim())
                  $(inp).parents('form').find('input[name="counsel_email"]').val(input_array[6].trim())
                  $(inp).parents('form').find('input[name="counsel_phone"]').val(input_array[7].trim())
                  
                  $(inp).parents('form').find('input[name="counsel_fax"]').val(input_array[8].trim())
                  $(inp).parents('form').find('input[name="counsel_extension"]').val(input_array[9].trim())
                  console.log('WEbiste',input_array[10].trim())
                  try {
                    
                    $(inp).parents('form').find('input[name="counsel_last_name"]').val(input_array[11].trim())
                    } catch (error) {
                      
                    }
                    $(inp).parents('form').find('input[name="attorney_id"]').val(input_array[12].trim())
                  inp.value = "";                  
  ...
  function search_filter_counsel_attorney(this_element){
      var $this = $(this_element);
      $.ajax({ 
            method: "GET",
            url: `/30/search_filter_counsel_attorney/`,
            ...
            success:function(response){
              ...
              autocomplete_counsel_attorney(this_element, response.data, response.data );
