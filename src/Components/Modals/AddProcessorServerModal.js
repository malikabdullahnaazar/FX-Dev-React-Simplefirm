...
import { addProcessServer, fetchProcessServerData } from "../../Redux/process-server/processServerSlice";
const initialState = {
  name: "",
  cost: "",
  address1: "",
...
function AddProcessorServerModal({
  handleClose,
  addProcessorServer
}) {
  ...
  const handleAttorneySubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        // dispatch(addProcessServer(responseData))
  ...
  return (
    <div
      className={`modal generic-popup generic-popup-scroll fade bd-example-modal-md ${addProcessorServer ? "show" : ""}`}
      style={{
        backgroundColor: "rgba(40, 80, 122, 0.5)",
        zIndex: 100000,
        display: addProcessorServer ? "flex" : "none",
