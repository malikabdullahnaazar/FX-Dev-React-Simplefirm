...
//U2024235:4:00AM
const AssignTodoSearchModal = ({ show, handleClose}) => {
  ...
  const currentCase = useSelector((state) => state.searchS.currentCase);
  const showAssignTodoModalMessage = useSelector((state) => state.searchS.showAssignTodoModalMessage);
...
// to populate the field with the value from outside the field
useEffect(()=>{
   setDescription(showAssignTodoModalMessage)
},[])
  ...
  async function createNotesTodo() {
    const taskData = {
      client_id: currentCase?.for_client?.id,
          ...
          <div className="modal-body">
            <div>
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                <p className="p-l-3" style={{ border: "none" }}>
              ...
              <div className="d-flex justify-content-between align-items-center m-b-5 p-l-3">
                <div className=" notes-text-area " style={{ border: '1px solid var(--primary)',height: '75px',zIndex: 18 }}>
                  <textarea
                    className="assign_todo_textarea anti-skew"
...
export default AssignTodoSearchModal;
