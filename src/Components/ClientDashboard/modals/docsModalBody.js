...
const ModalBodyDocs = () => {
    ...
    const handleFileChange = async (e) => {
        ...
        setFiles(selectedFiles); // Optionally you can remove this if not using the state elsewhere
        await handleUploadDocs(selectedFiles);
    };

    const handleUploadDocs = async (uploadFiles) => {
        ...
        try {
            const response = await api.post(
                `${origin}/api/client/upload-doc/${clientId}/${currentCaseId}/`,
                formData,
                {
                    headers: {
                ...
                <div className='section__content section__content--p30'>
                    <div className="w-100 flex-container p-2" id="upload-doc" style={{ display: "flex", height: "32vh", justifyContent: "center", alignItems: "center", border: "2px dashed rgb(0, 0, 0, 0.2)", borderRadius: "15px", padding: "20px" }}>
                        <div className="drag-area" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "40px" }}>
                            ...
                            <header style={{ margin: "10px 0", fontWeight: "500", fontSize: "30px", color: "rgb(3, 4, 94)" }}>Upload Documents</header>
                            
                            <div className="form-group">
                                <input type="file" className="form-control-file" id="docFileUpload" name="files[]" multiple required style={{ display: "none" }} onChange={handleFileChange} />
                                <label htmlFor="docFileUpload" className="btn btn-primary cp-display-ib-cursor">Choose And Upload Files</label>
                            </div>
                        </div>
                    </div>
                </div>
